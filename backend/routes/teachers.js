...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    let collection = await db.collection(process.env.TEACHERS_DATA);
    data = await collection.find({ name: { $exists: true } });
...
router.get('/delete', (req, res) => {
  res.json({
    message: 'Delete a teacher from the Teachers list in the database'
  })
})
